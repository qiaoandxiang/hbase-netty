...
import org.apache.hadoop.hbase.util.CompactedConcurrentSkipListMap.MemoryUsage;
import org.apache.hadoop.hbase.util.CompactedTypeHelper.KVPair;
import org.apache.hadoop.hbase.util.TestCompactedConcurrentSkipList.BehaviorCollections.RandomBehavior;
...
public class TestCompactedConcurrentSkipList {
  ...
  static int META_PAGE_SIZE = 16 * 1024;
  ...
  static class TypeHelper implements CompactedTypeHelper<Integer, String> {
    ...
    @Override
    public CompactedTypeHelper.KVPair<Integer, String> decomposte(byte[] data,
        int offset, int len) {
      Assert.assertTrue(len > 4);
      int intkey = getIntFromArray(data, offset);
      String value = new String(data, offset + 4, len - 4);
      return new KVPair<Integer, String>(new Integer(intkey), value);
    }
  ...
  static interface KVGenerator {
    abstract public KVPair<Integer, String>[] getNextGroup(Random random);

    abstract public KVPair<Integer, String> getNextKV(Random random);

    abstract public void validate(int key, String value);

    // whether this generator can provide a group of kv
    public boolean groupKV();

    public boolean singleKV();
  }
    ...
    static public KVGenerator arbitaryKVGenerator(final int keyRange,
      ...
      return new KVGeneratorBase(true, true) {
        ...
        public KVPair<Integer, String> getNextKV(Random random) {
          ...
          return new KVPair<Integer, String>(getHashWithKeyRange(keyRange,
              value), value);
        }
      };
    }
  ...
  class Operators {
    final List<Runnable> functions = new ArrayList<Runnable>();
  ...
  private CompactedConcurrentSkipListMap<Integer, String> createMapForTest(
    ...
    if (!largeMap) {
      return new CompactedConcurrentSkipListMap<Integer, String>(
          new TypeHelper(), CompactedConcurrentSkipListMap.PageSetting.create()
              .setDataPageSize(DATA_PAGE_SIZE).setHeapKVThreshold(BIG_KV_THRESHOLD));
    } else {
      CompactedConcurrentSkipListMap.PageSetting ps = CompactedConcurrentSkipListMap.PageSetting
          .create();
  ...
  public void testPutAndGet() {
    ...
    m.put(2, "hello");
    m.put(9, "year");
    Assert.assertEquals(m.get(-50), null);
    Assert.assertEquals(m.get(99), null);
    Assert.assertEquals(m.get(2), new String("hello"));
    Assert.assertEquals(m.get(9), new String("year"));
    Assert.assertEquals(m.get(100), new String("thanks"));
  }
        ...
        public void run() {
          ...
          for (int c = 0; c < COUNT; ++c) {
            ...
            for (int j = 0; j < group.length; ++j) {
              m.put(group[j].key, group[j].value);
            }
            for (int j = 0; j < group.length; ++j) {
              String value = m.get(group[j].key);
              kvg.validate(group[j].key.intValue(), value);
            }
          }
        }
      });
  ...
  static class MapContext {
    final int keyRange;
    final int totalThread = 0; // 0 means single thread;
  ...
  public void testEqual() throws Throwable {
    ...
    Assert.assertTrue(mForTest.equals(mForStand));
    Assert.assertTrue(mForStand.equals(mForTest));
  }
  ...
  static class BehaviorCollections {
    ...
    static class MapRange {
      ...
      void reverse() {
        ...
        boolean bt = this.startInclusive;
        this.startInclusive = this.endInclusive;
        this.endInclusive = bt;
      }
    }
    ...
    public static class KeySetIterator extends AbstractBehavior {
      ...
      public InputType doBehavior(MapContext mc) {
        ...
        while (iter2.hasNext()) {
          ...
          Integer ret1 = iter1.next();
          Integer ret2 = iter2.next();
    ...
    public static class RandomBehavior {
      Random r = getRandom();
      NavigableMap<InputType, List<AbstractBehavior>> behaviors = new TreeMap<InputType, List<AbstractBehavior>>();
      NavigableMap<InputType, NavigableMap<Integer, AbstractBehavior>> weightedBehaviors = new TreeMap<InputType, NavigableMap<Integer, AbstractBehavior>>();
      NavigableMap<InputType, Integer> totalWeight = new TreeMap<InputType, Integer>();
      ...
      public RandomBehavior(boolean multiThread) {
        ...
        try {
          ...
          for (int i = 0; i < classes.length; ++i) {
            Class<?> cls = classes[i];
            if (AbstractBehavior.class.isAssignableFrom(cls)
                && !cls.equals(AbstractBehavior.class)) {
              ...
              List<AbstractBehavior> list = behaviors
                  .get(newBehavior.getType());
              if (list == null) {
                list = new ArrayList<BehaviorCollections.AbstractBehavior>();
                behaviors.put(newBehavior.getType(), list);
              }
              list.add(newBehavior);

              System.out.println("Created new behavior "
                  + newBehavior.getName());
            }
          }
        ...
        for (Entry<InputType, List<AbstractBehavior>> entry : behaviors
          ...
          NavigableMap<Integer, AbstractBehavior> wm = new TreeMap<Integer, AbstractBehavior>();
          int totalw = 0;
      ...
      public Behavior nextBehavior(InputType inputType) {
        int totalw = totalWeight.get(inputType).intValue();
  ...
  static class Performace {
    Map<String, Integer> behaviorToOps = new ConcurrentHashMap<String, Integer>();
    Map<String, Long> behaviorToLatency = new ConcurrentHashMap<String, Long>();

    void addOps(String behavior, long latencyInNs) {
      Integer o = behaviorToOps.get(behavior);
      o = o == null ? 1 : o.intValue() + 1;
      behaviorToOps.put(behavior, o);

      Long l = behaviorToLatency.get(behavior);
      l = l == null ? latencyInNs : l.longValue() + latencyInNs;
  ...
  public void testRandomBehavior() {
    ...
    RandomBehavior rb = new RandomBehavior(false);
    for (int i = 0; i < COUNT; ++i) {
